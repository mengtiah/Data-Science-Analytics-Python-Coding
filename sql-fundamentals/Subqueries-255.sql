...
FROM recent_grads
where Major_category in (SELECT Major_category FROM (SELECT Total, SUM(Total), Major_category FROM recent_grads group by Major_category order by 2 desc limit 5))
