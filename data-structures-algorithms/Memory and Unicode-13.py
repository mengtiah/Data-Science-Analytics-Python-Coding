...
def clean_statement(row):
    clean_row = [s for s in row if ord(s) in good_characters]
    clean_row_str = "".join(clean_row)
    return clean_row_str

    
sentences_cia['cleaned_statement']=sentences_cia['statement'].apply(clean_statement)
...
statement_tokens = list(combined_statements.split(' '))
...
def most_common_func(year):
    new = sentences_cia[sentences_cia['year']==year]
    combined_statements = " ".join(sentences_cia["cleaned_statement"])
    statement_split = list(combined_statements.split(" "))
    token = [s for s in statement_split if len(s)>=5]
    counter = Counter(token)
    return counter.most_common(2)
    
common_2000= most_common_func('2000')
common_2002 =most_common_func("2002")
common_2013 = most_common_func("2013")