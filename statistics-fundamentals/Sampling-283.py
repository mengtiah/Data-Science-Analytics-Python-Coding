...
sample_means=[]
discrepancy=[]
paremeter=wnba['PTS'].mean()
wnba = pd.read_csv('wnba.csv')
for i in range(100):
    sample=wnba['PTS'].sample(10,random_state=i)
    statistic=sample.mean()
    sample_means.append(statistic)
    diff=paremeter-statistic
    discrepancy.append(diff)

plt.scatter(x=range(1,101),y=sample_means)
plt.axhline(paremeter)
plt.show()
...
position_most_points=max(points_per_position,key=points_per_position.get) #Getting key with maximum value in dictionary
